"""Step 1: Extract all rows for 2,441 missing serials from SQLite, save to file."""
...
for i in range(0, len(missing), BATCH):
    ...
    if (i+BATCH) % 1000 == 0 or i + BATCH >= len(missing):
        print(f"  {min(i+BATCH, len(missing))}/{len(missing)} serials, {len(all_rows):,} rows ({time.time()-t0:.1f}s)")
conn.close()
...
print(f"Saved {len(all_rows):,} rows to backfill_raw.json")
